create or replace
   function oradev23.ddl(p_type varchar2, p_name varchar2, p_owner varchar2 default user) return clob is
   ...
   begin
      DBMS_OUTPUT.PUT_LINE ('Function DDL begins');
      DBMS_OUTPUT.PUT_LINE (upper(p_type));
      DBMS_OUTPUT.PUT_LINE (upper(p_name));
      DBMS_OUTPUT.PUT_LINE (upper(p_owner));
      dbms_lob.createtemporary(res,true);
      dbms_lob.createtemporary(ddl,true);
  
      -- select ltrim(dbms_metadata.get_ddl(upper(p_type),upper(p_name),upper(p_owner)),chr(10)||chr(13)||' ')
      select ltrim(sys.dbms_metadata.get_ddl(upper(p_type),upper(p_name),upper(p_owner)),chr(10)||chr(13)||' ')
      into   ddl
      from   dual;
